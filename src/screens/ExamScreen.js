...
async function uploadToCloudinary(uri, type) {
  ...
  form.append('file', { uri, type: mime, name: `exam.${ext}` });
  form.append('upload_preset', UPLOAD_PRESET);
  form.append('folder', 'ps_govindpur/exam');
...
function UploadBlock({ docId, title, accent }) {
  ...
  const save = async (newEntry) => {
    await setDoc(doc(db, 'exam', docId), newEntry);
    setEntry(newEntry);
  };
      ...
      {loading ? (
        ...
        entry.kind === 'image' ? (
          <View style={s.imageCard}>
            <Image source={{ uri: entry.url }} style={s.examImage} resizeMode="contain" />
...
export default function ExamScreen({ navigation }) {
  ...
  let titleHi = 'परीक्षा', titleEng = 'Examination';
  if (view === 'datesheet') { titleHi = 'डेट शीट'; titleEng = 'Date Sheet'; }
  if (view === 'marks' && selClass === null) { titleHi = 'परीक्षा अंक'; titleEng = 'Exam Marks'; }
  if (view === 'marks' && selClass !== null) { titleHi = CLASSES[selClass].hi; titleEng = CLASSES[selClass].eng; }

  const goBack = () => {
    if (view === 'marks' && selClass !== null) { setSelClass(null); return; }
    if (view !== null) { setView(null); return; }
    navigation.goBack();
  };
  ...
  return (
    <View style={[s.root, { paddingTop: insets.top }]}>
        ...
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 40 }}>
          <View style={s.section}>
            <LinearGradient colors={['#0F2347','#1A3A6B','#1E4D8C']} style={s.hero}>
              <View style={s.heroIcon}><Ionicons name="school" size={28} color={COLORS.gold} /></View>
              <Text style={s.heroTitle}>परीक्षा विवरण</Text>
              <Text style={s.heroSub}>अंक व डेट शीट · सत्र 2026-27</Text>
            </LinearGradient>
          </View>
          <View style={s.section}>
            <TouchableOpacity activeOpacity={0.88} onPress={() => setView('marks')}>
              <LinearGradient colors={['#7C3AED','#9F67FF']} style={s.bigCard}>
                <View style={s.bigIcon}><Ionicons name="ribbon" size={26} color="#fff" /></View>
                <View style={{flex:1}}>
                  <Text style={s.bigTitle}>परीक्षा अंक</Text>
                  <Text style={s.bigSub}>Exam Marks · कक्षावार</Text>
                </View>
                <Ionicons name="chevron-forward" size={22} color="#fff" />
              </LinearGradient>
            </TouchableOpacity>
            <TouchableOpacity activeOpacity={0.88} onPress={() => setView('datesheet')}>
              <LinearGradient colors={['#C2410C','#E07B39']} style={s.bigCard}>
                <View style={s.bigIcon}><Ionicons name="calendar" size={26} color="#fff" /></View>
                <View style={{flex:1}}>
                  <Text style={s.bigTitle}>डेट शीट</Text>
                  <Text style={s.bigSub}>Date Sheet</Text>
                </View>
                <Ionicons name="chevron-forward" size={22} color="#fff" />
              </LinearGradient>
            </TouchableOpacity>
      ...
      {view === 'datesheet' && (
        <UploadBlock docId="datesheet" title="डेट शीट" accent="#C2410C" />
      )}

      {view === 'marks' && selClass === null && (
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 40 }}>
          <View style={s.section}>
            <View style={s.secHead}><View style={[s.secBar,{backgroundColor:'#7C3AED'}]} /><Text style={s.secTitle}>कक्षा चुनें</Text></View>
      ...
      {view === 'marks' && selClass !== null && (
        <UploadBlock docId={`marks-class${CLASSES[selClass].n}`} title={`${CLASSES[selClass].hi} — अंक`} accent={CLASSES[selClass].color} />
      )}
    </View>
  );
}
...
const s = StyleSheet.create({
  ...
  secBar:       { width:4, height:22, borderRadius:2, marginRight:10 },
  ...
  bigCard:      { flexDirection:'row', alignItems:'center', gap:14, borderRadius:16, padding:18, marginBottom:12, elevation:3, shadowColor:'#000', shadowOpacity:0.18, shadowRadius:7 },
  bigIcon:      { width:48, height:48, borderRadius:14, backgroundColor:'rgba(255,255,255,0.2)', alignItems:'center', justifyContent:'center' },
  bigTitle:     { color:'#fff', fontSize:18, fontFamily:'NotoSansDevanagari_700Bold' },
  bigSub:       { color:'rgba(255,255,255,0.85)', fontSize:12, marginTop:2, fontFamily:'NotoSansDevanagari_400Regular' },
  ...
  examImage:    { width:'100%', height:440 },
